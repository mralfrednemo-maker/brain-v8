# Brief: Produce a Unified Master Design & DOD Document for the Thinker Platform
...
- **DECIDE** — the models converge toward a verdict
...
Consider the two versions of Design and DOD documents provided below. After analyzing and deeply understanding the purpose of this platform, create one final document by merging the best features and optimal solutions into one Master document where the design and the DOD will be detailed.

Specifically:

1. **Read and understand all four documents completely** before making any decisions. Understand what each mechanism does, why it exists, and how it serves the platform's core purpose.

2. **For each area where V3.0 and V3.0B diverge**, select the stronger mechanism — the one that is more specific, more enforceable, more auditable, and better serves the platform's purpose. Where both versions offer complementary features, merge them.

3. **Produce a single Master document** that contains:
   - **Part 1: Design** — the complete platform design with all mechanisms, pipeline stages, schemas, and architectural decisions
   - **Part 2: Definition of Done** — the complete DOD with acceptance criteria, failure modes, required schemas, and Gate 2 rules for every mechanism in Part 1

   The DOD must cover every design element. No design feature should exist without a corresponding testable DOD requirement.

4. **Do not invent new features.** Only combine, select, and merge from what exists in the four source documents.

5. **Flag any irreconcilable conflicts** between the documents that you cannot resolve by selection.

## Epistemic Override

All four documents are provided in full below. You have everything you need. Do not flag missing context.

---

## DESIGN-V3.md (complete)
