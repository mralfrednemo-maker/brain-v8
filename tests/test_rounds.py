...
class TestRound1:
    ...
    async def test_r1_calls_4_models(self, mock_llm):
        ...
        result = await execute_round(mock_llm, round_num=1, brief="Test brief")
    ...
    async def test_r1_prompt_contains_only_brief(self, mock_llm):
        ...
        await execute_round(mock_llm, round_num=1, brief="My test brief about JWT")
        ...
        assert "evidence" not in prompt.lower() or "RESEARCH CONTEXT" not in prompt
    ...
    async def test_r1_handles_model_failure(self, mock_llm):
        ...
        result = await execute_round(mock_llm, round_num=1, brief="Test")
...
class TestRound2Prompt:
    ...
    def test_r2_prompt_includes_r1_views(self):
        prompt = build_round_prompt(
            ...
            evidence_text="{E001} [HIGH] Some fact\nSource: https://example.com",
