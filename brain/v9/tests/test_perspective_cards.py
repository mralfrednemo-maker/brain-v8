...
import asyncio
import pytest
from brain.types import BrainError, CoverageObligation, PerspectiveCard, TimeHorizon
...
def test_extract_cards_from_r1():
    from brain.perspective_cards import extract_perspective_cards
    r1_texts = {
    ...
    cards = _run(extract_perspective_cards(r1_texts))
    assert len(cards) == 4
    assert all(c.primary_frame for c in cards)
    assert all(c.failure_mode for c in cards)
    # All native — no LLM client needed
    for c in cards:
        assert all(v == "native" for v in c.field_provenance.values())


def test_extract_cards_missing_fields_no_client():
    """DOD §7.3: missing fields without LLM client → legacy majority threshold."""
    from brain.perspective_cards import extract_perspective_cards
    r1_texts = {
    ...
    with pytest.raises(BrainError, match="perspective cards"):
        _run(extract_perspective_cards(r1_texts))
